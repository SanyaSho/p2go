...
//	MessageEnd();
	...
	CUsrMsg_ChangePaintColor msg;
	msg.set_entity(this->entindex());
	msg.set_paintpower(nIndex);
	SendUserMessage(filter, UM_ChangePaintColor, msg);
}
...
void CWeaponPaintGun::GiveAllPaintPowers_Client()
{
...
static void CC_GiveAllPaintPowers()
{
	CBaseEntity* pFoundEnt = gEntList.FindEntityByClassname( NULL, "weapon_paintgun" );
	while( pFoundEnt )
	{
...
static ConCommand giveallpaintpowers( "giveallpaintpowers", CC_GiveAllPaintPowers );
