...
bool CPropJeep::CheckWater( void )
{
	...
	for ( int iWheel = 0; iWheel < JEEP_WHEEL_COUNT; ++iWheel )
	{
		...
		{
			m_WaterData.m_bWheelInWater[iWheel] = ( UTIL_PointContents_HL2( m_WaterData.m_vecWheelContactPoints[iWheel] ) & MASK_WATER ) ? true : false;
	...
	m_WaterData.m_bBodyInWater = ( UTIL_PointContents_HL2( vecEnginePoint ) & MASK_WATER ) ? true : false;
...
void CPropJeep::CheckWaterLevel( void )
{
	...
	if ( pEntity && pEntity->IsPlayer() )
	{
		...
		bool bEyes = ( UTIL_PointContents_HL2( vecAttachPoint ) & MASK_WATER ) ? true : false;
		if ( bEyes )
		{
		...
		GetAttachment( iAttachment, vecAttachPoint, vecAttachAngles );
		bool bFeet = ( UTIL_PointContents_HL2( vecAttachPoint ) & MASK_WATER ) ? true : false;
		if ( bFeet )
		{
