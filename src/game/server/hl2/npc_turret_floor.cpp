...
bool CNPC_FloorTurret::IsValidEnemy( CBaseEntity *pEnemy )
{
	...
	if ( UTIL_PointContents_HL2(EyePosition()) & MASK_SHOT )
		return false;
	...
	{
		CPortal_Base2D* pPortalBase = FInViewConeThroughPortal(pEnemy);

		if (pPortalBase)
		{
			// Assuming CPortal_Base2D is a base class of CProp_Portal
			CProp_Portal* pPortal = dynamic_cast<CProp_Portal*>(pPortalBase);
