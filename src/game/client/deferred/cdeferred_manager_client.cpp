...
//	const bool bDeferredEnabled = CommandLine() && CommandLine()->FindParm( "-nodeferred" ) == 0;
	const bool bDeferredEnabled = true;
...
/*	if (!bDeferredEnabled) {
	...
	if ( bSM30 || bForceDeferred )
	{
		...
		if ( bGotDefShaderDll )
		{
			...
			ConVarRef r_drawskybox("r_drawskybox");
			r_drawskybox.SetValue("0"); //PB TODO = fix the skybox?
	...
	if ( !m_bDefRenderingEnabled )
	{
		...
		Warning( "Your hardware does not seem to support shader model 3.0. If you think that this is an error (hybrid GPUs), add -forcedeferred as a start parameter.\n" );
		Error("Your hardware does not seem to support shader model 3.0. If you think that this is an error (hybrid GPUs), add -forcedeferred as a start parameter.");
		g_pCurrentViewRender = new CViewRender();
	}
	else
	{
