...
public:
	...
	void SetChapterIndex( int nIndex )
	{
		...
		// Assuming m_nChapterIndex is 1-based
		if (m_nChapterIndex > nNumCommentaryMaps)
		{
			int chapterIndex = m_nChapterIndex - nNumCommentaryMaps;

			bool bIsLocked = (chapterIndex > m_pDialog->GetNumAllowedChapters());
			if (bIsLocked)
			{
				m_bLocked = true;

				pChapterTitle = g_ChapterTitles[chapterIndex - 1]; // Adjust index to be 0-based

				// Truncate the title to preserve "Chapter ?"
				const wchar_t* pHeaderPrefix = wcsstr(pChapterTitle, L"\n");
				if (pHeaderPrefix)
				{
					// Truncate the title, want to preserve "Chapter ?"
					// This assumes that the newline character separates the title from additional text
					int truncatedLength = pHeaderPrefix - pChapterTitle;
					V_wcsncpy(chapterString, pChapterTitle, truncatedLength);
					chapterString[truncatedLength] = L'\0';
		...
		if ( nChapterIndex > 0 )
		{
			...
			if ( pMapName && pMapName[0] )
			{
				...
				if ( IsGameConsole() && bSavesAllowed )
				{
					...
					// Issue map command
					char mapCommand[256];
					V_snprintf(mapCommand, sizeof(mapCommand), "map %s", pMapName);
